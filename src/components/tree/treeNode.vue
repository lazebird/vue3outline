<template>
  <div class="tree-node-container">
    <node-content />
    <div v-if="nextShow" class="tree-node-children" :style="{ paddingLeft: indent }">
      <tree-node v-for="(child, idx) of nodeData.children" :key="idx" :node-data="child" />
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    props: { nodeData: { type: Object, required: true } },
    data() {
      return {
        tree: false,
        level: 0,
